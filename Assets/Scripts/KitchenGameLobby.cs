...
public class KitchenGameLobby : MonoBehaviour
{
    ...
    private void HandleHeartbeat()
    {
        if (IsLobbyHost())
        {
            ...
            {
                float heartbeatTimerMax = 15f; // Heartbeat interval in seconds
    ...
    public async void CreateLobby(string lobbyName, bool isPrivate)
    {
        ...
        try
        {
            joinedLobby = await LobbyService.Instance.CreateLobbyAsync(lobbyName, KitchenGameMultiplayer.MAX_PLAYERS_AMOUNT, new CreateLobbyOptions { IsPrivate = isPrivate });
    ...
    public async void JoinWithId(string lobbyId)
    {
        ...
        {
            Debug.LogError($"Failed to join lobby with code: {e.Message}");
